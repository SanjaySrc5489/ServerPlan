...
/**
 * GET /api/auth/token
 * Get current user's permanent API token
 */
 ...
 * Generate a permanent API token for user (only if they don't have one)
 */
router.post('/generate-token', verifyToken, checkExpiration, async (req, res) => {
    try {
        const user = req.user;
        const ipAddress = req.ip;
        const userAgent = req.headers['user-agent'];
        ...
        if (user.apiToken) {
            return res.status(400).json({
                success: false,
                error: 'Token already exists. Permanent tokens cannot be regenerated.',
                token: user.apiToken
            });
        }

        // Generate a permanent, secure token
        // Format: prefix_randomBytes for easy identification
        const tokenBytes = crypto.randomBytes(32).toString('hex');
        const apiToken = `pgt_${tokenBytes}`; // pgt = Permanent Gallery Token
        ...
        await auditLog(user.id, 'api_token_generated', 'user', user.id, {
            tokenPrefix: apiToken.substring(0, 8)
        }, ipAddress, userAgent, true);

        console.log(`[AUTH] API token generated for user: ${user.username}`);

        res.json({
            success: true,
            token: apiToken,
            message: 'Permanent API token generated. This token cannot be changed or deleted.'
        });
    } catch (error) {
 ...
 * Validate an API token and return user info (for Android app)
 */
router.post('/validate-token', async (req, res) => {
    try {
        ...
        const apiToken = token || (authHeader && authHeader.startsWith('Bearer ') ? authHeader.slice(7) : null);
        ...
        // Generate a JWT for the device to use
        const jwtToken = jwt.sign(
            ...
            JWT_SECRET,
            { expiresIn: '365d' } // Long-lived for device use
        );
        ...
        res.json({
            ...
            userToken: jwtToken, // For device registration
            signatureSecret: user.signatureSecret
        });
    } catch (error) {
